...
DEFAULT_OUTPUT_DIR = Path("results/gemini_arrow_frame_benchmark")
...
# Current Gemini model-page pricing, used only for approximate cost reporting.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "with a Gemini multimodal model."
        )
    )
    ...
    parser.add_argument(
        ...
        default="original",
        help="Recorded in the run config for parity with the GPT benchmark.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Use a positive integer to enable extra thinking."
        ),
    )
...
def local_image_to_base64(path: Path) -> tuple[str, str]:
    if not path.exists():
...
def input_image_item(path: Path, detail: str) -> dict[str, Any]:
    mime_type, encoded = local_image_to_base64(path)
    return {
        "inline_data": {
            "mime_type": mime_type,
            "data": encoded,
...
def build_request_payload(
    *,
    ...
    if use_json_schema:
        generation_config["responseJsonSchema"] = build_json_schema_format()
...
def extract_response_text(response_json: dict[str, Any]) -> str:
    ...
    for candidate in response_json.get("candidates", []):
        ...
        if finish_reason:
            texts.append(f"[NO_TEXT] finishReason={finish_reason}")
    return "\n".join(texts).strip()
...
def call_model_for_view(
    *,
    ...
    response_model = response_json.get("modelVersion", model)
    ...
    return {
        ...
        "response_id": None,
        "response_status": (
            (response_json.get("candidates") or [{}])[0].get("finishReason")
            or "completed"
        ),
